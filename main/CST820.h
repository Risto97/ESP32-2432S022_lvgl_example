...
{
public:
    CST820(gpio_num_t sda_pin = GPIO_NUM_NC, gpio_num_t scl_pin = GPIO_NUM_NC, gpio_num_t rst_pin = GPIO_NUM_NC, gpio_num_t int_pin = GPIO_NUM_NC);

    void begin(void);
    bool getTouch(uint16_t *x, uint16_t *y, uint8_t *gesture);

private:
    gpio_num_t _sda, _scl, _rst, _int;
